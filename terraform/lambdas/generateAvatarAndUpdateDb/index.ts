...
export const handler: S3Handler = async (event: S3Event) => {
  ...
  try {
    ...
    for (const [key, value] of Object.entries(BREAKPOINTS)) {
      ...
      const dstKey = `avatar_gen/${path}/${key}_${imageId}.webp`
      ...
      if (breakpoint < width || breakpoint < height || size > 10000) {
        ...
        const { error } = await supabase
          .from('User')
          .update({ image: cloudfrontURL + dstKey })
          .eq('id', userId)

        if (error) {
          console.error(error)
          throw new Error('supabase errorb')
        }
      }
    }
  } catch (error) {
